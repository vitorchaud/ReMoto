...
public class Configuration implements Serializable
{
	...
	public NeuronVO getNeuronByType(String cdNucleus, String type) 
	{
		for(int i = 0; i < neuronTypes.size(); i++)
		{
			NeuronVO neu = (NeuronVO)neuronTypes.get(i);
			...
			*/
			...
			if(category.equals("MN")){
				for(int i = 0; i < neuronTypes.size(); i++)
				{
					...
					if( neu.getCategory().equals(category) && cdNucleus.equals(neu.getCdNucleus()) ){
	...
	{
		getNeuronByType(cdNucleus, type).setQuantity( quantity );
		getNeuronByType(cdNucleus, type).setActive( true );
	}
