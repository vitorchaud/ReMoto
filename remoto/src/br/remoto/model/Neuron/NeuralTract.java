...
public class NeuralTract extends Neuron
{
	...
	double timeStep = miscellaneous.getStep() / 1000;
	...
	public NeuralTract(NeuronVO neu, int index, Miscellaneous misc, boolean createSignal)
	{
		...
		//System.out.println("Creating neural tract");
		//System.out.println("createSignal: " + createSignal);
		//System.out.println("neu.getAmp(): " + neu.getAmp());
		...
		//System.out.println("signal.getAmp(): " + signal.getAmp());
		...
		try
		{
			...
			*/
			if(distribution.equals(ReMoto.poisson ) ){
				...
				double rate = 1000 / instantMean;
				
				
				//System.out.println("rate: " + rate + "   timeStep: " + timeStep + "   rate * timeStep: " + rate * timeStep);
				
				if( mean > ReMoto.T_TOLERANCE || ( signal != null && t > signal.getTini() && t < signal.getTfin() + ReMoto.T_TOLERANCE ) )
				{
					//double probability = Math.exp(-rate * timeStep) * rate * timeStep;
						
					double probability = rate * timeStep;
				...
				double instantMean = meanValue(t);
				double lambda = 1000 / instantMean;	// Tenho dúvida!
...
//		if( ret < miscellaneous.getStep() )
		if( ret < 0.1 )
			...
			ret = 0.1; //avoid high frequencies (upper limit = 10 kHz)
