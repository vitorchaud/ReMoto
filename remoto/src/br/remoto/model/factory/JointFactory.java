...
import br.remoto.model.Musculotendon.Muscle.ExtrafusalMuscle.Models.Hill2;
...
public class JointFactory 
{
			...
			GolgiTendonOrgan[] gtos, MusculotendonSuperClass musculotendons[])
	{
		...
		for(int x = 1; x < neurons.length; x++)
		{
			...
			if( conf.getNeuronByType(cdNucleus, ReMoto.S).isActive() )
				numMNs = conf.getNeuronByType(cdNucleus, ReMoto.S).getQuantity();
			
			if( conf.getNeuronByType(cdNucleus, ReMoto.FR).isActive() )
				numMNs += conf.getNeuronByType(cdNucleus, ReMoto.FR).getQuantity();
			
			if( conf.getNeuronByType(cdNucleus, ReMoto.FF).isActive() )
				numMNs += conf.getNeuronByType(cdNucleus, ReMoto.FF).getQuantity();
			...
			if(muscle_model.equals(ReMoto.Raikova)){
				...
				muscle = new Hill2(conf, cdNucleus, motorunits, muscle_model);
			}
