...
public class SynapsesFactory 
{
	...
	public void createSynapses(Configuration conf, Neuron neurons[][])
	{
		...
		for(int x = ReMoto.indDT + 1; x < neurons.length; x++)
		{
			...
			for(int n = 0; n < neurons[x].length; n++)
			{
				...
				if( neu.getCategory().equals( ReMoto.MN ) )
					...
					rcNucleus.add( neu );
				else if( neu.getType().equals( ReMoto.IaIn ) || neu.getType().equals( ReMoto.IbIn ) )
					inNucleus.add( neu );
			}
			...
			for(int m = 0; m < mnNucleus.size(); m++)
			{
				...
				for(int d = 0; d < nt.size(); d++)
				{
					...
					// Place DTs equally distributed in relation to MN pool
		...
		// --------------------------------------
		// 4) Ia's/Ib's synapses among the nuclei
		// 		a) Ia over MN 
		// 		b) Ia over IaIn 
		// 		b) Ib over IbIn 
		// --------------------------------------
		...
		for(int y = 1; y < neurons.length; y++)
		{
			...
			if( iaNucleus.size() == 0 && ibNucleus.size() == 0 )
				continue;
			...
			for(int x = 1; x < neurons.length; x++)
			{
				...
				// There is no synapse to create
				if( mnNucleus.size() == 0 && iaInNucleus.size() == 0 && ibInNucleus.size() == 0 )
					continue;
...
/*
	...
	{
		//1.	Dividiu-se o paralelepípedo em 3 seções, a 1a seção mais rostral, a 2a seção intermediária e a 3a seção mais caudal.
		//2.	Na 1a seção, posicionam-se os MNs LG tipo FF.
		//3.	Na 2a seção, posicionam-se os MNs LG tipo S e MNs MG-SOL tipo FF
		//4.	Na 3a seção, posicionam-se os MNs MG-SOL tipo S.
		//5.	MNs LG tipo FR são posicionados aleatoriamente na 1a e 2a seção.
		//6.	MNs MG-SOL tipo FR são posicionados aleatoriamente na 2a e 3a seção.
		//7.	INs LG são distribuídos homogeneamente na 1a e 2a seção.
		//8.	INs MG-SOL são distribuídos homogeneamente na 2a e 3a seção.
