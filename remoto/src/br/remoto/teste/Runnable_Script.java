...
public class Runnable_Script 
{
	...
	{
		Runnable_Script teste = new Runnable_Script();
	...
	public void run() throws FileNotFoundException
	{
		...
		conf = simDAO.getConfiguration(15);
		...
		conf.setPrimaryBag1Gain(20000);
		conf.setPrimaryBag2AndChainGain(10000);
		conf.setSecondaryBag2AndChainGain(7250);
		...
		dataPath = ReMoto.path + "simulation";
		graphicPath = ReMoto.path + "simulation";
		...
		for(int k = 1; k <= numOfSimulations; k++){
			...
			//generateAndStoreData("somaticPotential", sim, dataPath, graphicPath, path, k);
	...
	public void generateLogOfScenario(int simCount, String dataPath, String path) throws FileNotFoundException{
	    ...
	    for(int i = 0; i < conf.getNeuronTypes(ReMoto.DT, ReMoto.TR).size(); i++){
	    	...
	    	printWriter.println("conf.getNeuronTypes(ReMoto.DT, ReMoto.TR).get(" + i + "): " + "\t" + 
    				"dt.getCategoryType(): " 	+ dt.getCategoryAndType()+ "\t" + 
